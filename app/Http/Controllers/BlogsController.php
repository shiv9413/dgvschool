...
class BlogsController extends Controller
{
    public function index(Request $request){
        $data = ModulesData::where('module_id', 9)->where('status','active');
        ...
        $arr['recent_data'] = ModulesData::where('module_id', 9)->where('status','active')->orderBy('id','desc')->take(3)->get(); 
    ...
    public function detail($slug){
    	...
    	$data['recent_data'] = ModulesData::where('module_id', 9)->where('id','!=',$data['blog']->id)->where('status','active')->orderBy('id','desc')->take(3)->get(); 
