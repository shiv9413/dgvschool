...
class ApplicantsController extends Controller
{
    ...
    public function index($id) {
        $applicants = Applicants::select(['*'])->where('job_id',$id)->paginate(10);
        return view('admin.applicants.index')->with('applicants',$applicants);
    }

    public function applicantProfile($id) {
        $applicant = Applicant::findorFail($id);
        return view('admin.applicants.profile')->with('applicant',$applicant);
    }

    public function applicant_home($id){
        $applicant = Applicant::findOrFail($id);
        Auth::guard('applicant')->login($applicant);
        return redirect(route('applicant.a.home'));
    }
    ...
    {
        $applicants = Applicants::select(['*'])->where('job_id',$request->job_id)->paginate(10);
        ...
        return Datatables::of($applicants)
                        ->filter(function ($query) use ($request) {
                            ...
                            if ($request->has('class') && !empty($request->class)) {
                                $query->where('class_id', '=', "{$request->get('class')}");
                            }
                        ...
                        ->addColumn('image', function ($students) {
                            $image = '<img style="width: 57%;" src="'.asset('/images/thumb/'.$students->attached_file).'" alt="">';
                            return $image;
                        })
                        ...
                        ->addColumn('class', function ($students) {
                               ...
                               return title($students->class_id); 
                        ...
                        ->addColumn('action', function ($students) {
                            $action = '<a href="'.route('admin.students.edit',[$students->id]).'" class="tabledit-edit-button btn btn-primary waves-effect waves-light" style="float: none;margin: 5px; font-size: 12px;"><span class="icofont icofont-ui-edit"></span>&nbsp Edit</a>
                                <a href="'.route('admin.students.delete',[$students->id]).'" class="tabledit-delete-button btn btn-danger waves-effect waves-light" style="float: none;margin: 5px; font-size: 12px;"><span class="icofont icofont-ui-delete"></span>&nbsp Delete</a>';
                            return $action;
                        })
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $student->delete();
        $request->session()->flash('message.added', 'success');
        $request->session()->flash('message.content', 'Student has been successfully Deleted!');
        return redirect(route('admin.students'));
    }
